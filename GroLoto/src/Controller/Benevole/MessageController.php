...
class MessageController extends AbstractController
{
    ...
    public function index(): Response
    {
        ...
        if ($email) {
            ...
            // INCLURE AUSSI les messages envoyés à 'ALL_BENEVOLES'
            $messages = $this->em->getRepository(ContactMessage::class)
                ->createQueryBuilder('cm')
                ->where('cm.parent_id IS NULL')
                ->andWhere('cm.destinataire = :email OR cm.email = :email OR cm.destinataire = :allBenevoles')
                ->andWhere('cm.masqueePour IS NULL OR cm.masqueePour NOT LIKE :emailPattern')
                ->setParameter('email', $email)
                ->setParameter('allBenevoles', 'ALL_BENEVOLES')
        ...
        return $this->render('benevole/message/messages.html.twig', [
            'messages' => $messages,
    ...
    #[Route('/envoyes', name: 'benevole_messages_sent')]
    ...
    #[Route('/nouveau', name: 'benevole_message_new', methods: ['POST'])]
    public function new(Request $request, MailerInterface $mailer): JsonResponse
    {
        ...
        // Utiliser ROLE_ADMIN pour le champ destinataire en BDD pour que tous les admins le voient
    ...
    #[Route('/{id}/delete', name: 'benevole_message_delete', methods: ['POST'])]
    public function delete(ContactMessage $message, Request $request): Response
    {
        ...
        if ($user->getEmail() !== $message->getEmail() && $user->getEmail() !== $message->getDestinataire()) {
            ...
            return $this->redirectToRoute('benevole_messages');
        }
    ...
    #[Route('/{id}/cloturer', name: 'benevole_message_close', methods: ['POST'])]
    ...
    #[Route('/{id}/repondre', name: 'benevole_message_reply', methods: ['POST'])]
    ...
    #[Route('/{id}/edit', name: 'benevole_message_edit', methods: ['POST'])]
    ...
    #[Route('/{id}/delete-message', name: 'benevole_message_delete_message', methods: ['POST'])]
    public function deleteMessage(Request $request, ContactMessage $message, EntityManagerInterface $entityManager): JsonResponse
    {
        if (!$request->isXmlHttpRequest()) {
            return $this->json(['success' => false, 'error' => 'Requête non autorisée'], 403);
        }
